...
This problem involves one method of cleaning up data by replacing missing values with reasonable values; in this case, data that is not available (NA) will be replaced with the mean of the variable. 
...
The following code chunk provides a means of seeing the process for cleaning the data one variable at a time with the means calculated separately and fed into the data frame individually. For cases in which the variable is a character, i.e. for the species variables, the "virginica" species has been used. 
...
This last code chunk then uses a function using the `if... if else... else` construct to designate changes to be made to either numeric or character variables. The resulting data table is printed.
